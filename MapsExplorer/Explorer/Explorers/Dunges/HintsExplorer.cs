...
public class HintsExplorer : ExplorerBase
{
	...
	public override void Work()
	{
		...
		for (int i = 0; i < _resultLines.Count; i++)
		{
			...
			if (line.Category != MapCategory.Рандом)
				continue;
			...
			{
				if (dunge.LogLine.Custom || dunge.IsCustom)
					counter = customCounter;
				else if (dunge.LogLine.DungeKind == DungeKind.Полуправды && dunge.StartKind == DungeKind.Полуправды)
					counter = halfCounter;
				else if (dunge.LogLine.DungeKind == DungeKind.Термодинамики && dunge.StartKind == DungeKind.Термодинамики)
					counter = thermoCounter;
				else if (dunge.LogLine.DungeKind == DungeKind.Спешки && dunge.StartKind == DungeKind.Спешки)
					counter = quickCounter;
				else if (dunge.LogLine.DungeKind == DungeKind.Густоты && dunge.StartKind == DungeKind.Густоты)
					counter = moreCounter;
				else if (dunge.LogLine.DungeKind != DungeKind.Загадки && dunge.StartKind != DungeKind.Загадки)
